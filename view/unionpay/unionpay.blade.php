...
@section ('main')
    ...
    <div class="content page-content-inner recharge-netbank">
        ...
        <form action="{{$confirm_internal["unionpay"]}}" method="post" id="J-form" target="_blank">
            <input type="hidden" name="_token" value="{{ csrf_token() }}"/>
            <input type="hidden" name="bank_code" value="UNIONPAY"/>
            <input type="hidden" name="deposit_mode" value="{{ UserDeposit::DEPOSIT_MODE_THIRD_PART }}"/>
            <input data-id="{{ $oBank->id }}" name="bank" value="{{ $oBank->id }}"
                   id="J-bank-name-{{ $oBank->identifier }}" type="hidden"/>
            <table width="100%" class="table-field">
                <tr>
                    <td align="right" style="width:480px;"><span class="field-name">充值方式：</span></td>
                    <td valign="bottom">
                        <span class="bank-unionpay" title="UNIONPAY支付" id="J-alipay-ico">
                        </span>
                    </td>
                </tr>
                <tr>
                    ...
                    <td>
                        ...
                        <br/>
                        <span class="tip">充值额度限定：最低 <span id="J-money-min">{{ $fMinLoad }}</span>元,最高 <span
                ...
                @if(count($aWeixinPayBank) > 0)
                    <tr>
                        <td colspan="2" align="center"><font color="red" size="4">温馨提示：您即将进入扫码支付页面，支付成功后，窗口将自动关闭，请勿手动关闭窗口，以免影响充值进度。</font>
                        </td>
                    </tr>
                @endif
            </table>
